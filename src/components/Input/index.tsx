...
import React, { useState, ChangeEvent, useEffect, MouseEvent, FocusEvent } from "react";
import { HTMLFormType, InputType } from "~/interfaces/forms.intf";

import "./style.scss";

const Input: React.FunctionComponent<InputType> = ({
  prependIcon,
  appendIcon,
  label = "Label",
  name = "inputName",
  ...
  return (
    <div
      ...
      className={`input${htmlFormType === HTMLFormType.textarea ? " input--textarea" : ""}${
        textInput !== "" ? " input--active" : ""
      }${onFocusInput ? " input--focus" : ""}${disabled ? " input--disabled" : ""}${
        errorStatus ? " input--error" : ""
      }${readOnly ? " input--read" : ""}`}
    >
      <label htmlFor={name} className={prependIcon ? "indent" : ""}>
        {label}
      </label>
      <div
        data-testid={"wrapper"}
      ...
      >
        ...
        {appendIcon ? (
          <i data-testid="append-icon">
            <FontAwesomeIcon icon={appendIcon} />
          </i>
        ) : null}
      </div>
      {errorStatus && <p className="input__error-message">{errorMessage}</p>}
    </div>
  );
};
