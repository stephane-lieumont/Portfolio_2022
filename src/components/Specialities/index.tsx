...
const Specialities: React.FunctionComponent<SpecialitiesProps> = ({
  specialities = [],
  contentLoaded = false,
}) => {
  return (
    <ul className="specialities" data-testid="specialities">
      {specialities.map((speciality, index) => (
        <li
          key={`speciality-${index}`}
          className={`specialities__item${
            contentLoaded ? ` specialities__item__reveal--${index}` : ""
          }`}
        >
