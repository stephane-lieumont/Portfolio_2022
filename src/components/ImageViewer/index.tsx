...
const ImageViewer: React.FunctionComponent<ImageViewerProps> = ({
  ...
  return imageData === undefined ? null : (
    <div
      className={`image-viewer${
        isOpen ? " image-viewer--open" : isAnimateClose ? " image-viewer--close" : ""
      }`}
    ...
    >
      ...
      <div
        className={`image-viewer__container${
          isAnimateChange ? " image-viewer__container--hide" : ""
        }${imageLoaded ? " loaded" : ""}`}
        style={{ animationDuration: `${duration / 2}ms` }}
      >
        <h2>
          {firstLetterUpper(imageDataSelected?.title ?? "")}{" "}
          <span>{imageDataSelected?.released.getFullYear()}</span>
        </h2>
        ...
        {!refreshImg && (
          <img
            ...
            src={imageDataSelected?.imgFile}
            alt={imageDataSelected?.imgAlt}
          />
        )}
        <i onClick={handleCloseModal} data-testid="btn-close">
          <FontAwesomeIcon icon={faClose} className={"image-viewer__container__close"} />
        </i>
      </div>
      <div className="image-viewer__controllers">
        <i data-testid="btn-previous" onClick={handlePreviousImg}>
          <FontAwesomeIcon icon={faArrowCircleLeft} />
        </i>
        <div>
          {imageIndex + 1} / {PortfolioImagesData.length}
        </div>
        <i data-testid="btn-next" onClick={handleNextImg}>
