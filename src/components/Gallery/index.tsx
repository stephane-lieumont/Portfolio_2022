import React, { createRef, Fragment, MouseEvent, SyntheticEvent, useMemo, useState } from "react";
...
const Gallery: React.FunctionComponent<GalleryProps> = ({
  ...
  const [imageLoadCounter, setImageLoadCounter] = useState<number>(0);
  ...
  const handleLoadImage = (e: SyntheticEvent<HTMLImageElement>) => {
    ...
    e.currentTarget.parentNode
      ?.querySelector(".gallery__grid__item__desc")
        ...
        {portfolioData.map((imageData, index) => (
          <div
            ...
            className={`
              gallery__grid__item 
              ${
                index % 5 === 0 && index < portfolioData.length - 3
                  ? "gallery__grid__item--double-row "
                  : index % 3 === 0 && index < portfolioData.length - 7
                  ? "gallery__grid__item--double-column "
          ...
          >
            {visible && (
              <Fragment>
                <img
                  className="loading"
                  src={imageData.imgFileThumb}
                  alt={imageData.imgAlt}
                  ref={refsById[imageData.id]}
