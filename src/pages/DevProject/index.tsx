...
const DevProject: React.FunctionComponent<PageProps> = ({ title = "titre de la page" }) => {
              ...
              <div>
                <ul className={`icon-custom__list reveal${imgLoaded ? " reveal--4" : ""}`}>
                  {dataProject?.technos.map((item, index) => (
                    <li key={`techno-${index}`} className="icon-custom">
                      <i
                        className={`icon-custom__container icon-custom--${item.toLowerCase()}`}
                      ></i>
                      <span>{firstLetterUpper(item)}</span>
                    </li>
                  ))}
                </ul>
