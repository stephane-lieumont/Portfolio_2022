...
const NavBarSlider: React.FunctionComponent<NavBarSliderProps> = ({
  ...
  useEffect(() => {
    ...
    slideIn.current
      ...
      .to(
        startPoints.current,
        {
          attr: { d: middlePoints.current?.getAttribute("d") || "" },
          duration: 0.2,
          ease: Elastic.easeInOut,
        },
        0
      )
      .addLabel("rectangleStart", ">")
      .to(
        startPoints.current,
        {
          attr: { d: startPoints.current?.getAttribute("d") || "" },
          duration: 0.7,
          ease: Elastic.easeInOut,
    ...
    slideOut.current
      .to(
        wrapper.current,
        { x: "100%", duration: 0.7, ease: Power3.easeInOut, onStart: () => setIsAnimated(true) },
        0
      )
      .to(
        startPoints.current,
        {
          attr: { d: endPoints.current?.getAttribute("d") || "" },
          duration: 0.2,
          ease: Power3.easeIn,
        },
        0
  ...
  useEffect(() => {
    windowSize.width < Number(Screen.screenXl.replace("px", ""))
      ? setMobileNav(true)
      : setMobileNav(false);
  }, [windowSize]);
  ...
  return (
    <div data-testid="navbar">
      <div
        ...
        className={`navbar__button${animated ? " navbar__button--animated" : ""}${
          ligth ? " navbar__button--ligth" : ""
        }${open ? " open" : ""}`}
        onClick={handleClick}
      >
      ...
      <div className={`navbar__slider${open ? " open" : ""}`} ref={wrapper}>
        ...
        >
          <path
            d="M 210,297 H 0 C 0,297 0,230.472 0,148.5 0,66.528001 0,0 0,0 h 210 z"
          ...
          <defs>
            <path
              d="M 210,297 H 49.560001 C 49.560001,297 0,230.472 0,148.5 0,66.528001 49.560001,0 49.560001,0 H 210 Z"
              id="end"
              ref={endPoints}
            ></path>
            <path
              d="M 210,297 H 0 C 0,297 51.660001,230.472 51.660001,148.5 51.660001,66.528001 0,0 0,0 h 210 z"
              id="middle"
              ref={middlePoints}
            ></path>
          </defs>
          ...
          <ul ref={menu}>
            {routeList.map(({ path, label, name, params }, index) =>
              path !== "*" && params?.mainMenuEnabled !== false ? (
                ...
                <li className={open ? `delay--${index}` : ""} key={name} onClick={handleClick}>
                  <NavLink to={path}>{label}</NavLink>
                </li>
              ) : null
            )}
